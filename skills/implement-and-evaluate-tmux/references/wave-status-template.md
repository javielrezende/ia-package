...
- **A coluna `PR` aceita as duas formas de forge.** Os exemplos acima usam URLs de GitHub (`.../pull/<N>`), mas o valor vem do campo `pr_url` do status file, que o `team-driver.sh` extrai do journal reconhecendo também `.../-/merge_requests/<N>` do GitLab. Copie a URL como ela veio e prefixe o número com a convenção do forge: `#<N>` no GitHub, `!<N>` no GitLab. Em texto corrido, use o termo do forge do projeto (veja `${CLAUDE_PLUGIN_ROOT}/references/forge.md` § 6); `PRs opened:` continua com esse rótulo nos dois casos.
